...
def main_menu_kb() -> ReplyKeyboardMarkup:
 ...
 [KeyboardButton(text="📦 Мои заказы"), KeyboardButton(text="i️ Помощь")],
 ],
...
@router.message(F.text == "i️ Помощь")
...
async def my_orders(message: Message):
 ...
 lines.append(f"No{order_id} • {product_short} • размер {size} • {label} • {created_at}")
 await message.answer("\n".join(lines))
...
async def admin_accept(callback: CallbackQuery):
 ...
 user_id = order[1]
 try:
 await bot.send_message(user_id, f"✅ Твоя заявка No{order_id} принята в работу!")
 except Exception:
 pass
...
async def admin_reject(callback: CallbackQuery):
 ...
 user_id = order[1]
 try:
 await bot.send_message(user_id, f"❌ Твоя заявка No{order_id} отклонена. Свяжись с менеджером для уточнения деталей.")
 except Exception:
 pass
 await callback.message.edit_text(callback.message.text + "\n\n❌ Отклонена")
